CREATE OR REPLACE TABLE OPEN_MARKET_NTU_CLASSIFICATION_V3_OUTPUT (
...
CREATE OR REPLACE TABLE OPEN_MARKET_NTU_CLASSIFICATION_V3_ERROR_LOG (
    ACCOUNT_FOLDER_NAME STRING,
...
def find_email_message_events(text):
    """
    Splits a PDF-exported email chain into distinct email messages.
    ...
    for m in line_iter:
        ...
        # Avoid recipient/header/signature lines
        ...
        # Must look like a message header, not just signature/contact block
        if not (
    ...
    # Deduplicate repeated detections from the same email
    dedup = {}
    ...
    for e in events:
        ...
        if key not in dedup:
            ...
            # Prefer From header over weaker rendered detection
            existing_rank = {
                "from_header": 3,
                "on_wrote": 2,
                "rendered_header": 1
    ...
    # Add body window for each event
...
def extract_conversation_metadata(combined_text):
    ...
    # Fallback only if no message events found
    if email_count == 0:
        fallback_dates = []
