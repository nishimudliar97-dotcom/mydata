...
def find_email_message_events(text):
    ...
    # 2. Gmail/forwarded style: On <date>, <person> wrote:
    wrote_patterns = [
    ...
    # Deduplicate repeated detection of same email
    dedup = {}
    ...
    # Build message body.
    # Body is cut at the next email marker to avoid quote detection leaking into old emails.
...
def has_convex_quote_language(body):
    """
    Detect only actual Convex quote / offer / terms emails.

    Do NOT return True for:
    - broker asking for terms
    - broker forwarding market terms
    - Convex saying "we will take a look"
    - Convex arranging a call
    - Convex saying "thanks / yes / noted"
    """
...
def extract_conversation_metadata(combined_text):
    ...
    # User-requested correction because PDF email extraction over-counts repeated markers.
...
- NEGOTIATION_FATIGUE is calculated by system: 1 only when adjusted EMAIL_COUNT_DETECTED > 20.
...
System-calculated only. The procedure will set 1 only when adjusted email count > 20.
...
Also mention the system-calculated quote timing and adjusted email count.
