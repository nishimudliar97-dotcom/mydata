...
CREATE OR REPLACE TABLE OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V3 (
...
CREATE OR REPLACE PROCEDURE RUN_OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V3(
    STAGE_ROOT STRING,
    MAX_FOLDERS NUMBER
)
RETURNS STRING
...
def get_already_processed_folders(session):
    rows = session.sql("""
        ...
        FROM OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V3
    """).collect()
...
def add_value(variable_key, raw_value, counters, raw_construction_counter, raw_to_normalized_mapping, count_value=1):
    raw_value = clean_text(raw_value)
...
def select_best_column(ws, header_row_number, candidate_columns):
...
def extract_variables_from_xlsx(scoped_file_url):
    ...
    for ws in wb.worksheets:
        ...
        for variable_key in VARIABLES.keys():
            ...
            selected_column = select_best_column(ws, header_row_number, candidate_columns)
...
def run(session, STAGE_ROOT, MAX_FOLDERS):
    ...
    for folder_name in folder_names_to_process:
        ...
        insert_sql = f"""
            INSERT INTO OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V3 (
                ACCOUNT_FOLDER_NAME,
                CONSTRUCTION_CODE,
            ...
            SELECT
                ...
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["CONSTRUCTION_CODE"])))}'),
                ...
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["YEAR_BUILT"])))}'),
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["YEAR_OF_LAST_MAJOR_RENOVATION"])))}'),
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["SQUARE_FOOTAGE"])))}'),
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["NUMBER_OF_STORIES"])))}'),
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["NUMBER_OF_BASEMENTS"])))}'),
                PARSE_JSON('{escape_sql_string(json.dumps(dict(counters["NUMBER_OF_BUILDINGS_AT_LOCATION"])))}'),
    ...
    return (
        f"Completed property variable extraction V3. "
        f"New folders processed in this run: {processed_folder_count}. "
        f"Files processed in this run: {total_files_processed}. "
        f"Folders already present in V3 output table and skipped: {len(already_processed_folders)}. "
...
CALL RUN_OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V3(
