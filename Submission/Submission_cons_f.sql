...
CREATE OR REPLACE TABLE OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V7 (
...
CREATE OR REPLACE PROCEDURE RUN_OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V7(
    STAGE_ROOT STRING,
    MAX_FOLDERS NUMBER,
...
def get_already_processed_folders(session):
    rows = session.sql("""
        ...
        FROM OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V7
    """).collect()
...
def run(session, STAGE_ROOT, MAX_FOLDERS, TARGET_FOLDER_KEYWORD):
    ...
    for folder_name in folder_names_to_process:
        ...
        insert_sql = f"""
            INSERT INTO OPEN_MARKET_PROPERTY_VARIABLE_EXTRACTION_V7 (
                ACCOUNT_FOLDER_NAME,
    ...
    return (
        f"Completed property variable extraction V7. "
        ...
        f"Folders already present in V7 output table and skipped: {len(already_processed_folders)}. "
        f"Construction variable logic removed: True. "
