...
CREATE OR REPLACE TABLE NTU_FOLDER_QUOTE_DISCOVERY_RESULTS_V2 (
    run_id STRING,
...
CREATE OR REPLACE PROCEDURE RUN_NTU_FOLDER_QUOTE_DISCOVERY_POC_V2(
    STAGE_PATH STRING,
...
11. Do not copy or reuse templated explanation wording. Write a case-specific explanation using the actual facts and figures from this conversation.

EXPLANATION REQUIREMENTS:
The "ntu_explanation" must be specific to this exact conversation.

Do not use generic or templated explanations.
Do not say only that "the broker preferred another market" or "there was pricing pressure" without explaining the actual evidence.

The explanation must include, wherever available:
- the specific premium figures discussed,
- the specific layer / limit / attachment structure discussed,
- the quoted line percentage,
- any competing market / lead market / broker target mentioned,
- any change between initial quote, revised quote, broker request, or competing terms,
- any conditions such as tied lines, signing in full, open period, NCG, SNDILR, subjectivities, deductibles, sublimits, wording, or coverage restrictions,
- why these specific details point to the selected NTU category.

If numerical figures are present in the conversation, include the important ones in the explanation.
If no numerical figures are present, explain the strongest non-numeric signals.

The explanation should read like a case-specific analyst note, not a generic classification sentence.
...
Do not treat them as a predefined taxonomy.
...
- "ntu_explanation": detailed case-specific explanation with figures, commercial movement, and why the category fits.
- "granular_factor_summary": concise factor-level summary of the main commercial drivers.
- "key_figures_discussed": list the important premiums, limits, layers, percentages, deductibles, targets, competing figures, or quote figures mentioned in the conversation.
- "quote_progression_summary": summarize how the quote, broker request, competing terms, or discussion changed across the conversation, including figures where available.
...
- "pricing_signals": pricing-related facts and figures.
- "market_competition_signals": competing market / lead market / broker market preference facts.
- "layer_capacity_structure_signals": layer, limit, attachment, line, and capacity facts.
- "terms_conditions_coverage_signals": terms, conditions, deductibles, sublimits, clauses, wording, coverage restriction facts.
- "timing_placement_signals": quote timing, open period, inception, placement progress, late quote facts.
- "broker_client_preference_signals": broker/client target, preference, relationship, or stated placement preference facts.
- "supporting_factors": secondary signals that support the selected category.
...
def build_retry_prompt(combined_text):
    return f"""
You previously answered with an unclear, insufficient, or generic category/explanation.
...
6. Make the explanation granular and specific to this case.
7. Do not use a templated explanation.
8. Include important figures where available: premium, layer, attachment, limits, line percentage, rate cut, deductibles, targets, or competing figures.
9. Explain the movement in discussion: initial broker request, quote offered, revised request, competing quote, changed layer, changed price, changed condition, or placement update.
10. Mention whether pricing, competing market, layer/capacity, terms, timing, broker/client preference, or risk appetite signals influenced the conclusion.
...
"""
            ...
            # Increase later if the model/context supports it and cost is acceptable.
            ...
            key_figures_value = result_json["key_figures_discussed"] if "key_figures_discussed" in result_json else []
            quote_progression_summary = result_json["quote_progression_summary"] if "quote_progression_summary" in result_json else ""
            ...
            insert_sql = f"""
                INSERT INTO NTU_FOLDER_QUOTE_DISCOVERY_RESULTS_V2
                SELECT
                    '{sql_escape(run_id)}',
                    CURRENT_TIMESTAMP(),
                    ...
                    '{sql_escape(json.dumps(key_figures_value))}',
                    '{sql_escape(quote_progression_summary)}',
...
TRUNCATE TABLE NTU_FOLDER_QUOTE_DISCOVERY_RESULTS_V2;

CALL RUN_NTU_FOLDER_QUOTE_DISCOVERY_POC_V2(
...
SELECT
    ...
    key_figures_discussed,
    quote_progression_summary,
    raw_llm_output:pricing_signals AS pricing_signals,
    raw_llm_output:market_competition_signals AS market_competition_signals,
    raw_llm_output:layer_capacity_structure_signals AS layer_capacity_structure_signals,
    raw_llm_output:terms_conditions_coverage_signals AS terms_conditions_coverage_signals,
    raw_llm_output:timing_placement_signals AS timing_placement_signals,
    raw_llm_output:broker_client_preference_signals AS broker_client_preference_signals,
    status,
    error_message,
    parse_errors
FROM NTU_FOLDER_QUOTE_DISCOVERY_RESULTS_V2
